/** Listing-mode HTML parse (hardened; replaces listing-parse.ts per §11 / §47). */

import type { CrawlerSource } from "@quizzeira/shared";
import { classifyExamKind, isConcursoEligible, normalizeOpenExam } from "@quizzeira/shared";
...
export function parseListingHtml(
  ...
  for (const match of html.matchAll(hrefRe)) {
    ...
    if (JUNK_TITLE_RE.test(title.trim())) continue;
    if (JUNK_URL_RE.test(href)) continue;
...
function examLikenessScore(listing: DiscoveredListing): number {
  ...
  const kind = classifyExamKind(listing.title, listing.href);
  if (!isConcursoEligible(kind)) score -= 20;
  return score;
}
...
export function filterOpenListings(
  ...
  const eligible = listings.filter((l) =>
    isConcursoEligible(classifyExamKind(l.title, l.href)),
  );
  const pool = eligible.length > 0 ? eligible : listings;
  let selected = pool;
  if (openPatterns.length > 0) {
    const open = pool.filter((l) => matchesAny(l.textBlob, openPatterns));
    if (open.length > 0) selected = open;
  }
  return [...selected].sort((a, b) => examLikenessScore(b) - examLikenessScore(a));
}
...
export function listingsToOpenRecords(listings: DiscoveredListing[], source: CrawlerSource) {
  return listings
    ...
    .filter((r) => isConcursoEligible(r.kind ?? "other"));
}
