-- Crawler redesign §29.2: DocumentRole, Section, Syllabus, KU, PreviousQuestion
...
CREATE TYPE "SectionRole" AS ENUM (
  'syllabus', 'vacancies', 'schedule', 'registration', 'exam_structure',
  'legal_disposition', 'instructional', 'content', 'legal_article',
  'question_block', 'answer_key', 'nav', 'other'
);

CREATE TYPE "EligibilityStatus" AS ENUM ('eligible', 'ineligible', 'parked');

ALTER TYPE "ExtractionStatus" ADD VALUE IF NOT EXISTS 'normalizing';
ALTER TYPE "ExtractionStatus" ADD VALUE IF NOT EXISTS 'classifying';
ALTER TYPE "ExtractionStatus" ADD VALUE IF NOT EXISTS 'needs_review';
...
ALTER TABLE "Document"
  ...
  ADD COLUMN IF NOT EXISTS "nearDuplicateOfId" TEXT,
...
CREATE INDEX IF NOT EXISTS "Document_role_status_idx" ON "Document"("role", "status");
...
CREATE INDEX IF NOT EXISTS "Section_documentId_role_idx" ON "Section"("documentId", "role");
...
CREATE INDEX IF NOT EXISTS "SyllabusNode_status_idx" ON "SyllabusNode"("status");
...
CREATE INDEX IF NOT EXISTS "PreviousQuestion_documentId_idx" ON "PreviousQuestion"("documentId");
...
CREATE UNIQUE INDEX IF NOT EXISTS "StageMetric_stage_bucketHour_decision_key"
  ON "StageMetric"("stage", "bucketHour", "decision");
CREATE INDEX IF NOT EXISTS "StageMetric_stage_bucketHour_idx" ON "StageMetric"("stage", "bucketHour");
