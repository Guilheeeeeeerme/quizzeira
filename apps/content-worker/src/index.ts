...
async function tick(): Promise<void> {
  if (contentEnv.extractionEnabled) {
    const pass = contentEnv.pipelineV2 ? runProcessPass : runExtractionPass;
    await pass().catch((err) =>
      logWarn("extraction/process pass errored", { worker: NAME, error: String(err) }),
    );
  }
  if (contentEnv.pipelineV2) {
...
logInfo("interval mode", {
  ...
  pipelineV2: contentEnv.pipelineV2,
