...
const DEMO_EMAIL = "guest@quizzeira-demo.local";
...
async function seedDemoMain(prisma: PrismaClient) {
  ...
  // Demo guest only — platform ADMIN is seed-platform / deploy bootstrap.
  ...
  if (!existing) {
    ...
    console.log(`Seeded demo guest ${DEMO_EMAIL}`);
  } else {
    await prisma.user.update({
      where: { email: DEMO_EMAIL },
      data: { passwordHash, displayName: "Demo Guest", role: "USER" },
    });
  }

  console.log("Demo seed completed (levels, question pools, guest user).");
}

export async function seedDemo(client?: PrismaClient): Promise<void> {
  const prisma = client ?? new PrismaClient();
  const owns = !client;
  try {
    await seedDemoMain(prisma);
  } finally {
