...
1. [ ] **durable job lease layer — remaining sub-steps** (tables+APIs: slices 9-10; first consumer: slice 11):
   ...
   - [ ] **1b. worker claim wiring — in progress.** Done: content-worker's `runImportPass` (slice 11). Still direct polling, oldest/simplest-first:
     - [ ] content-worker `runProcessPass` (apps/content-worker/src/stages/process.ts) — next up, same shape as import (claim → do work → complete/fail).
     - [ ] content-worker `runPlannerPass`, embedding pass, generation pass.
     - [ ] discovery-crawler's passes — different shape (crawls a Source / runs a TopicQuery, not "process one row"); design job `kind`s for those before converting (e.g. `crawl_source`, `run_topic_query`) rather than reusing `import_artifact`'s shape.
