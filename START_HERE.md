...
1. [ ] **durable job lease layer — remaining sub-steps** (content-api's `Job` table + claim API landed as slice 9; see Done table):
   - [ ] **1a. discovery-api job table**: same `Job` shape/migration pattern in discovery-api's own Postgres (`quizzeira_discovery`), own `/internal/jobs/*` routes — do not share content-api's table cross-plane.
   - [ ] **1b. content-worker claim wiring**: convert `runImportPass`/`runProcessPass`/`runPlannerPass`/embedding/generation passes from direct entity-status polling to `POST /internal/jobs/claim` + `complete`/`defer`/`fail`, one pass at a time (verify each pass's existing tests still pass before moving to the next) so a bad rollout is bisectable.
   - [ ] **1c. dead-letter visibility**: `GET /internal/jobs/dead` exists (slice 9) but nothing surfaces it yet — wire into the §10.3 dashboard work (item 7) rather than building a one-off view now.
